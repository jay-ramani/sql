...
CASE
	...
	END as prod_qty_type_condensed
, CASE
	WHEN product_name like '%pepper%' OR product_name like '%PEPPER%'
		THEN 1
	ELSE 0
	END as pepper_flag
...
SELECT vendor.vendor_id, vendor_booth_assignments.booth_number, count( vendor_booth_assignments.booth_number ) as [count_of_renting_the_booth]
...
SELECT customer_first_name, customer_last_name, sum( quantity * cost_to_customer_per_qty ) as [total_spending_per_customer]
...
HAVING total_spending_per_customer > 2000;
...
SELECT customer.customer_id, customer_first_name, customer_last_name, sum( quantity * cost_to_customer_per_qty ) as [total_spending_per_customer_in_april_2022]

FROM customer
INNER JOIN customer_purchases
	on customer.customer_id = customer_purchases.customer_id
WHERE
	strftime('%m', customer_purchases.market_date) = '04' AND strftime('%Y', customer_purchases.market_date) = '2022'
